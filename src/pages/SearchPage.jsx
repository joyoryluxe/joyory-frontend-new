...
const SearchPage = () => {
  ...
  const buildQueryParams = useCallback((currentCursor = null, limitOverride = null) => {
    ...
    p.append("limit", limitOverride ? String(limitOverride) : (searchTerm ? "500" : "9"));
  ...
  const fetchProducts = useCallback(async (currentCursor = null, reset = false) => {
    try {
      ...
      if ((searchTerm || isCustomSort) && reset) {
        // Loop to fetch all pages of matching products when a search term or custom sort is supplied
        let hasMorePages = true;
        let pageCursor = currentCursor;
        const productMap = new Map();

        while (hasMorePages) {
          const queryString = buildQueryParams(pageCursor, 50);
  ...
  const fetchMoreProducts = useCallback(async () => {
    ...
    const isCustomSort = filters.sort && filters.sort !== 'recent';
    if (searchTerm || isCustomSort) {
      if (visibleCount < allProducts.length) {
        setIsFetchingMore(true);
        setTimeout(() => {
          setVisibleCount((prev) => {
            const next = prev + 9;
            if (next >= allProducts.length) setHasMore(false);
            return next;
          });
          setIsFetchingMore(false);
        }, 200);
  ...
  }, [cursor, isFetchingMore, hasMore, fetchProducts, searchTerm, filters.sort, visibleCount, allProducts.length]);
  ...
  }, [filteredProducts, searchTerm, filters.sort, visibleCount]);
