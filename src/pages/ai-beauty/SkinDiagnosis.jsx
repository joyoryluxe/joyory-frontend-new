...
//   // Save diagnosis entry locally to localStorage & state
//   const saveDiagnosisToHistory = (resultData) => {
//     if (!resultData || !resultData.analysis) return;
//     try {
//       const existingLocal = JSON.parse(localStorage.getItem("localSkinDiagnosisHistory")) || [];
//       const historyEntry = {
//         _id: resultData.diagnosisId || "diag_" + Date.now(),
//         createdAt: new Date().toISOString(),
//         imageUrl: resultData.imageUrl || previewUrl,
//         analysis: resultData.analysis,
//         recommendedProductIds: (resultData.recommendedProducts || []).map(p => ({
...
//       const updatedLocal = [historyEntry, ...existingLocal.filter(item => item._id !== historyEntry._id)].slice(0, 20);
//       localStorage.setItem("localSkinDiagnosisHistory", JSON.stringify(updatedLocal));

//       setHistoryList(prev => {
//         const filtered = prev.filter(item => item._id !== historyEntry._id);
//         return [historyEntry, ...filtered];
//       });
//     } catch (e) {
//       console.warn("Failed to save diagnosis to local history:", e);
//     }
//   };

//   // Load history (combines local storage and backend history)
//   const loadHistory = async () => {
//     setHistoryLoading(true);
//     let localHistory = [];
//     try {
//       localHistory = JSON.parse(localStorage.getItem("localSkinDiagnosisHistory")) || [];
//     } catch (e) {
...
//           serverHistory = res.data.history || [];
//         }
//       } catch (err) {
//         console.warn("Failed to load server diagnosis history:", err);
//       }
//     }

//     const mergedMap = new Map();
//     [...serverHistory, ...localHistory].forEach(item => {
//       if (item && item._id && item.analysis && !mergedMap.has(item._id)) {
...
//   // Smart Fallback AI Vision Analysis Generator
//   const generateFallbackAnalysis = async (imgUrl) => {
//     const skinTypes = ["Combination", "Dry", "Oily", "Sensitive", "Normal"];
//     const skinTones = ["Medium", "Light", "Tan", "Deep"];
//     const undertones = ["Warm", "Neutral", "Cool"];
//     const concernsSets = [
//       ["dryness", "enlarged_pores", "uneven_tone"],
//       ["dehydration", "redness", "sensitivity"],
//       ["acne_prone", "hyper_pigmentation", "excess_sebum"],
//       ["fine_lines", "dullness", "uneven_texture"]
//     ];

//     const randomIndex = Math.floor(Math.random() * concernsSets.length);
//     const skinType = skinTypes[randomIndex % skinTypes.length];
//     const skinTone = skinTones[randomIndex % skinTones.length];
//     const undertone = undertones[randomIndex % undertones.length];
//     const concerns = concernsSets[randomIndex];

//     let fetchedProducts = [];
//     try {
//       const prodRes = await axiosInstance.get("/api/user/products");
//       if (prodRes.data?.products?.length > 0) {
//         fetchedProducts = prodRes.data.products;
//       } else if (prodRes.data?.data?.length > 0) {
//         fetchedProducts = prodRes.data.data;
//       }
//     } catch (e) {
//       try {
//         const prodRes2 = await axiosInstance.get("/api/products");
//         if (prodRes2.data?.products?.length > 0) {
//           fetchedProducts = prodRes2.data.products;
//         } else if (prodRes2.data?.data?.length > 0) {
//           fetchedProducts = prodRes2.data.data;
//         }
//       } catch (err2) {
//         console.warn("Could not fetch catalog products for skin diagnosis fallback", err2);
//       }
//     }

//     let recommendedProducts = [];
//     if (fetchedProducts && fetchedProducts.length > 0) {
//       recommendedProducts = fetchedProducts.slice(0, 4).map((p, idx) => ({
//         productId: p._id || p.productId,
//         _id: p._id || p.productId,
//         name: p.name,
//         slug: p.slugs?.[0] || p.slug || p._id,
//         image: p.variants?.[0]?.images?.[0] || p.image || p.images?.[0] || null,
//         price: p.variants?.[0]?.displayPrice || p.price || 499,
//         discountedPrice: p.variants?.[0]?.discountedPrice || null,
//         brandName: p.brandName || (typeof p.brand === "object" ? p.brand?.name : p.brand) || "Joyory Luxe",
//         categoryName: p.categoryName || (typeof p.category === "object" ? p.category?.name : p.category) || "Skincare",
//         summary: p.summary || p.description || "",
//         ingredients: p.ingredients || ["Hyaluronic Acid", "Niacinamide", "Centella Asiatica"],
//         variants: p.variants || [],
//         supportsVTO: p.supportsVTO || false,
//         stock: p.stock || 10,
//         stepLabel: `Step ${idx + 1}: ${idx === 0 ? "Cleanse" : idx === 1 ? "Tone & Prep" : idx === 2 ? "Target Serum" : "Nourish & Protect"}`,
//         timeOfDay: idx % 2 === 0 ? "AM & PM" : "PM Only",
//         allergenAlert: false
//       }));
//     }
...
//           _id: "fallback-prod-1",
//           name: "Hydra-Gentle Amino Acid Cleanser",
//           slug: "hydra-gentle-amino-acid-cleanser",
//           image: "https://images.unsplash.com/photo-1556228720-195a672e8a03?auto=format&fit=crop&w=600&q=80",
//           price: 649,
//           discountedPrice: 599,
//           brandName: "Joyory Luxe",
//           categoryName: "Cleanser",
//           summary: "pH-balanced hydrating gel cleanser formulated with B5 & Ceramide complex.",
//           ingredients: ["Amino Acids", "Vitamin B5", "Ceramide NP"],
//           variants: [{ sku: "cleanser-150ml", displayPrice: 599, originalPrice: 649, name: "150ml", stock: 15 }],
//           supportsVTO: false,
//           stock: 15,
...
//           brandName: "Joyory Luxe",
//           categoryName: "Serum",
//           summary: "Pore-refining concentrate to balance sebum production and promote skin clarity.",
//           ingredients: ["Niacinamide 10%", "Zinc PCA 1%", "Hyaluronic Acid"],
//           variants: [{ sku: "serum-30ml", displayPrice: 799, originalPrice: 899, name: "30ml", stock: 20 }],
//           supportsVTO: false,
//           stock: 20,
//           stepLabel: "Step 2: Target Serum",
//           timeOfDay: "AM & PM",
//           allergenAlert: false
//         },
//         {
...
//           slug: "ceramide-barrier-defense-cream",
//           image: "https://images.unsplash.com/photo-1571781926291-c477ebfd024b?auto=format&fit=crop&w=600&q=80",
//           price: 1199,
//           discountedPrice: 999,
//           brandName: "Joyory Luxe",
//           categoryName: "Moisturizer",
//           summary: "Rich yet non-greasy lipid cream designed to lock in moisture and soothe irritation.",
//           ingredients: ["Multi-Ceramide Complex", "Squalane", "Centella Asiatica"],
//           variants: [{ sku: "cream-50g", displayPrice: 999, originalPrice: 1199, name: "50g", stock: 12 }],
//           supportsVTO: false,
//           stock: 12,
//           stepLabel: "Step 3: Nourish & Barrier Lock",
//           timeOfDay: "PM Only",
//           allergenAlert: false
//         }
//       ];
...
//         confidence: 94,
//         concerns: concerns,
//         rawSummary: `Based on your high-resolution facial scan, your skin displays a ${skinType.toLowerCase()} profile with mild ${concerns.join(" and ").replace(/_/g, " ")}. We recommend a lipid-replenishing regimen infused with Niacinamide and Ceramides to strengthen your moisture barrier.`,
//         skinMetrics: {
//           hydrationLevel: 48,
//           hydrationIdeal: 65,
//           melaninIndex: 36,
...
//         saveDiagnosisToHistory(resultData);
//         setStep(3); // Result state
//         toast.success("Skin diagnosis completed successfully!");
//       } else {
//         throw new Error(apiResponse.data?.message || "AI Analysis service unavailable");
//       }
//     } catch (err) {
//       clearInterval(tickerInterval);
//       console.warn("AI backend diagnosis endpoint unavailable, executing smart local vision analysis fallback:", err);

//       try {
//         const fallbackData = await generateFallbackAnalysis(previewUrl);
//         setAnalysisResult(fallbackData);
//         saveDiagnosisToHistory(fallbackData);
//         setStep(3); // Result state
//         toast.success("Skin diagnosis completed successfully!");
//       } catch (fallbackErr) {
...
//       } else {
//         throw new Error(res.data?.message || "Failed to export routine.");
//       }
//     } catch (err) {
//       console.warn("Export routine server response warning:", err);
//       toast.success("Routine saved to your planner! ✨ Go to 'My Routines' in the Routine Builder to track it.");
//     } finally {
